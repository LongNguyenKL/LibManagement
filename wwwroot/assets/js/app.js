...
async function getBooks() {
    ...
    try {
        let response;
        if (searchType === "genre") {
            response = await fetch(`${apiUrl}/books/genre/${encodeURIComponent(searchValue)}`);
        } else if (searchType === "bookid") {
            response = await fetch(`${apiUrl}/books/${encodeURIComponent(searchValue)}`);
        }
        ...
        // If the searchType is "genre", books will be an array; otherwise, wrap the single book object in an array.
        ...
        books.forEach(book => {
            ...
            row.innerHTML = `
                ...
                <td>
                    ...
                    <button onclick="updateBookDetails(${book.bookId})">Update</button>
                </td>
            `;
...
async function generateBooksByPublishedYearChart() {
    try {
        const response = await fetch(`${apiUrl}/Book/PublishedYear`);
...
async function generateMemberDemographicsChart() {
    try {
        const response = await fetch(`${apiUrl}/Member/Gender`);
        if (!response.ok) throw new Error(`Failed to fetch member demographics: ${response.status}`);

        const data = await response.json();
        const demographics = data.map(d => d.demographics);
